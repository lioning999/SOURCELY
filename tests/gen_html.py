...
DESC = {
    ...
    'winportUrl': '店铺Winport链接', 'sourceKeyword': '搜索关键词', 'scrapedAt': '抓取时间戳',
...
for path, desc in DESC.items():
    ...
    items.append((ORDER[g], path, val, desc))
items.sort(key=lambda x: (x[0], x[1]))

groups = {'产品': [], '供应商': []}
for _, path, val, desc in items:
    g = classify(path)
...
<title>1688实测返回指标 — 卡通可爱手编织小蒲扇</title>
<style>
...
.sok{color:var(--ok);font-weight:700}.sn{color:var(--bad);font-weight:700}
...
.dok{background:var(--ok)}.dno{background:var(--bad)}
</style>
</head>
<body>
<h1>1688 Wholesale Scraper — 实测返回指标</h1>
<div class="meta">
测试商品: <a href="''' + data.get('detailUrl','#') + '''" target="_blank">''' + data.get('title','') + '''</a><br>
供应商: ''' + supplier.get('companyName','') + ''' | offerId: ''' + data.get('offerId','') + ''' | 2026-07-20<br>
数据来源: Apify <code>zen-studio/1688-wholesale-scraper</code> 实测 | <a href="apify_raw_response.json">原始JSON(13KB)</a>
</div>
...
html += '''<div class="legend"><span><span class="dot dok"></span>有值</span><span><span class="dot dno"></span>null/空</span>点击分组标题折叠</div>'''

for gname in ['产品', '供应商']:
    gitems = groups[gname]
    # 过滤掉 null/空值 的行
    gitems = [(p, v, d) for p, v, d in gitems if not (v is None or v == '' or v == [])]
    oks = len(gitems)
...
<div class="gbody">'''
    for path, val, desc in gitems:
        ...
        badge = ''
        if v_type == 'n':
            badge = '<span style="font-size:9px;padding:1px 5px;border-radius:6px;font-weight:700;background:#FDEEEB;color:var(--bad)">null</span>'
        elif v_type == 'no':
...
open('tests/apify_fields_viewer.html', 'w', encoding='utf-8').write(html)
print(f'Done: {sum(len(v) for v in groups.values())} fields')
for g in ['产品', '供应商']:
    # 过滤后计数
    gitems = [(p, v, d) for p, v, d in groups[g] if not (v is None or v == '' or v == [])]
    print(f'  {g}: {len(gitems)} OK')
